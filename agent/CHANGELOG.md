...
- 🔧 **Config par défaut plus claire** : Utilise `"CHANGE_ME"` au lieu d'URLs invalides
  ...
  - Le collector et config_sync ignorent automatiquement `backend_url` si elle contient `CHANGE_ME` ou `example.com`
  - Fallback intelligent vers `api_url` si `backend_url` n'est pas configurée
